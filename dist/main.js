...
var computeGridLayout = function computeGridLayout(domTree) {
  var gridMatrix = [],
      ...
      parseTemplete = function parseTemplete(template) {
    ...
    return rowStart - 1 >= 0 && rowStart - 1 < templateRows.length && rowEnd - 2 >= 0 && rowEnd - 2 < templateRows.length && colStart - 1 >= 0 && colStart - 1 < templateColumns.length && colEnd - 2 >= 0 && colEnd - 2 < templateColumns.length;
      ...
      addCoordinatesToCells = function addCoordinatesToCells() {
    var i,
        j,
        item,
        usedX = 0,
    ...
    for (i = 0; i < formattedRows.length; i++) {
      ...
      placeChildrenInGrid = function placeChildrenInGrid() {
    children.forEach(function (child) {
    ...
    var rowsWithSize, columnsWithSize, i, j;
    columnsWithSize = tsa.set('tracks', formattedColumns).set('items', children.map(function (c) {
      return {
        start: c.style.gridColumnStart,
        end: c.style.gridColumnEnd,
        size: c.style.width
      };
    })).set('containerSize', height).resolveTracks();
    formattedColumns = columnsWithSize;
    rowsWithSize = tsa.set('tracks', formattedRows).set('items', children.map(function (r) {
      return {
        start: r.style.gridRowStart,
        end: r.style.gridRowEnd,
        size: r.style.height
      };
    })).set('containerSize', width).resolveTracks();
  ...
  placeChildrenInGrid(); // Size the rows and tracks

  inflateGridCells(); // Calling this second time to ensure that if some item's min-content-contribution has changed

  inflateGridCells(); // Adds x,y coordinates to each cell depending on position and dimensions
